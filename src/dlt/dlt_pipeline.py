...
def bronze_orders():
    return (
        spark.read.format("csv")  # noqa: F821  — `spark` is a DLT runtime global
        .option("header", "true")
...
@dlt.expect_or_drop("valid_purchase_timestamp", "order_purchase_timestamp IS NOT NULL")
@dlt.expect_or_fail("no_future_orders", "order_purchase_timestamp < current_timestamp()")
def silver_orders():
    return (
