...
def compute_category_trend(df_gold_revenue: DataFrame) -> DataFrame:
    ...
    """
    w_mom = Window.partitionBy("product_category_name").orderBy("order_year", "order_month")

    return (
        df_gold_revenue.select(
            "order_year",
            "order_month",
            "order_month_str",
        ...
        .withColumn("prev_month_revenue", lag("total_revenue", 1).over(w_mom))
        .withColumn(
        ...
        .withColumn("prev_month_orders", lag("total_orders", 1).over(w_mom))
        .withColumn(
            "mom_order_growth_pct",
            spark_round(
                (col("total_orders") - col("prev_month_orders")) / col("prev_month_orders") * 100, 2
            ),
        )
